...
class Fragment(object):
    ...
    fragment_identifier : int
        Unique identifier of the fragment assigned with
        :func:`~list_of_blobs.ListOfBlobs.compute_fragment_identifier_and_blob_index`
        and :func:`~list_of_blobs.ListOfBlobs.compute_crossing_fragment_identifier`
    start_end : tuple
        (starting_frame,  ending_frame) of the fragment
    blob_hierarchy_in_starting_frame : int
        Hierarchy of the blob in the starting frame of the fragment
        (the hierarchy if compute bottom-top, left-right).
    images : list
        List of images associated to every blob represented in the fragment and
        ordered according to the frame they have been segmented from
    ...
    is_an_individual : bool
        True if the image has been classified as representing an individual
        by :func:`~crossing_detector.detect_crossings`
    is_a_crossing : bool
        True if the image has been classified as representing a crossing by
        :func:`~crossing_detector.detect_crossings`
    ...
    is_in_a_global_fragment : bool
        True if the fragment is contained in a global fragment
        (see :class:`~globalfragment.GlobalFragment`)
    used_for_training : bool
        True if the fragment has been used to train the idCNN in the
        finerprinting protocol used to track the video
    accumulated_globally : bool
        True if the fragment's images have been accumulated with global
        strategy as references during the fingerprinting protocol used to track the video
    accumulated_partially : bool
        True if the fragment's images have been accumulated with partial
        strategy as references during the fingerprinting protocol used to track the video
    accumulation_step : int
        Accumulation step in which the fragment's images have been accumulated
    accumulable : bool
        True if the fragment is eligible for accumulation i.e. it is contained
        in a global fragment object (see :class:`~globalfragment.GlobalFragment`)
    used_for_pretraining : bool
        True if the fragment has been used to pretrain the idCNN in the third
        protocol (see :func:`~pre_trainer.pre_train`)
    acceptable_for_training : bool
        True if the fragment satisfies the conditions in
        :meth:`~accumulation_manager.AccumulationManager.check_if_is_acceptable_for_training`
    frequencies : ndarray
        array with shape [1, number_of_animals]. The ith component is the
        number of times the fragment has been assigned by the idCNN
        (argmax of the softmax predictions) to the identity i
    P1_vector : ndarray
        array with shape [1, number_of_animals] computed from frequencies by
        :meth:`compute_identification_statistics`
    P2_vector : ndarray
        array with shape [1, number_of_animals] computed from the P1_vector
        and considering all the identified fragment overlapping (in frames)
        with self by :meth:`compute_identification_statistics`
    certainty : float
        certainty of the identification of the fragment computed according to
        :meth:`compute_certainty_of_individual_fragment` (depends only on the
        predictions of the idCNN for the fragment's images)
    certainty_P2 : float
        certainty of the identification of the fragment computed according to
        :meth:`compute_P2_vector`
    is_certain : bool
    ...
    identities_corrected_closing_gaps : list
        Identity assigned to the fragment while solving the crossing if it
        exists else None
    user_generated_identity : int
        Identity assigned to the fragment by the user during validation if it
        exists else None
    final_identities : list
        Final identities of the fragment. It corresponds to
        :attr:`user_generated_identities` if it exist otherwise to the identity
    ...
    ambiguous_identities : list
        List of possible identities in case the assignment is ambiguous
        (two or more identity can be assigned to the fragment with the same
        certainty)
    potentially_randomly_assigned : bool
        True if :attr:`certainty` is below random wrt the number of images in
        the fragment
    non_consistent : bool
        True if exist a fragment that overlaps in frame with self whose
        identity is the same as self
    number_of_images : int
        number of images composing the fragment
    has_enough_accumulated_coexisting_fragments : bool
        the partial accumulation strategy assumes that the condition of
        :attr:`has_enough_accumulated_coexisting_fragments` holds
    distance_travelled : float
        distance travelled by the individual in the fragment
    coexisting_individual_fragments : list
        List of fragment objects that coexist (in frames) with self and that
