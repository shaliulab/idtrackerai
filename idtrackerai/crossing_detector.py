...
from idtrackerai.network.crossings_detector_model.get_crossings_data_set import (
    CrossingDataset,
)
from idtrackerai.network.crossings_detector_model.network_params_crossings import (
    NetworkParams_crossings,
)
from idtrackerai.network.crossings_detector_model.crossings_detector_model import (
    ConvNetwork_crossings,
)
from idtrackerai.network.crossings_detector_model.train_crossings_detector import (
    TrainDeepCrossing,
)
from idtrackerai.network.crossings_detector_model.get_predictions_crossings import (
...
def detect_crossings(
    ...
    if use_network:
        ...
        training_set = CrossingDataset(
            list_of_blobs.blobs_in_video, video, scope="training"
        )
        if not training_set.there_are_crossings:
            ...
            training_set.get_data(
                sampling_ratio_start=0, sampling_ratio_end=0.9
            )
            ...
            validation_set.get_data(
                sampling_ratio_start=0.9, sampling_ratio_end=1.0
            )
            ...
            crossings_detector_network_params = NetworkParams_crossings(
                number_of_classes=2,
        ...
        if not trainer.model_diverged:
            ...
            training_set = None
            logger.debug(
                "Freeing memory. Validation and training crossings sets deleted"
            )
