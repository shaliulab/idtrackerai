...
class ListOfBlobs(object):
    ...
    def interpolate_from_user_generated_centroids(self, id, start_frame, end_frame):
            ...
            for blob in blobs_in_frame:
                if not isinstance(blob.final_identity, list) and blob.final_identity == id:
                    ...
                    if blob.user_generated_centroid is not None:
                        user_generated_centroids.append(blob.user_generated_centroid)
                    else:
                    ...
                    if blob.user_generated_centroid is not None:
                        user_generated_centroids.append(blob.user_generated_centroid[index])
                    else:
                        user_generated_centroids.append((np.nan, np.nan))
        assert len(user_generated_centroids) == len(blobs_of_id) == end_frame-start_frame
        ...
        for i, blob in enumerate(blobs_of_id):
