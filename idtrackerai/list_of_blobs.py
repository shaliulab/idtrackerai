...
class ListOfBlobs(object):
    ...
    def compute_overlapping_between_subsequent_frames(
        self, n_jobs=conf.NUMBER_OF_JOBS_FOR_CONNECTING_BLOBS
    ):

        if n_jobs == 1:
            return (
                self.compute_overlapping_between_subsequent_frames_non_parallel()
            )
        if n_jobs != 1:
            return self.compute_overlapping_between_subsequent_frames_parallel(
                n_jobs
            )
    ...
    @staticmethod
    def compute_overlapping_between_subsequent_frames_single_thread(
