...
class ListOfBlobs(object):
    ...
    def reconnect(self):
        ...
        for frame_i in tqdm(
            range(1, self.number_of_frames), desc="re-connecting blobs"
        ):
            for (blob_0, blob_1) in itertools.product(
                self.blobs_in_video[frame_i - 1], self.blobs_in_video[frame_i]
            ):
        ...
        for blobs_in_frame in tqdm(
            ...
            for blob in blobs_in_frame:
                if blob.fragment_identifier is None and blob.is_an_individual:
                    ...
                    blob.non_shared_information_with_previous = 1.0
                    if (
                        len(blob.next) == 1
    ...
    def compute_overlapping_between_subsequent_frames(self):
        ...
        for frame_i in tqdm(
            ...
            set_frame_number_to_blobs_in_frame(
                self.blobs_in_video[frame_i - 1], frame_i - 1
            )
        ...
        set_frame_number_to_blobs_in_frame(
            self.blobs_in_video[frame_i], frame_i
        )
    ...
    def check_maximal_number_of_blob(
        ...
        if len(frames_with_more_blobs_than_animals) > 0:
            logger.error(
                "There are frames with more blobs than animals, this can be detrimental for the proper functioning of the system."
            )
            logger.error(
    ...
    def next_frame_to_validate(self, current_frame, direction):
        ...
        if not (
            ...
            raise Exception(
                "The frame number must be between 0 and the number of frames in the video"
            )
        if direction == "future":
