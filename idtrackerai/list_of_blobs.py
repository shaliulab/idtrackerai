...
class ListOfBlobs(object):
    ...
    def interpolate_from_user_generated_centroids(self, video, id, start_frame, end_frame):
        ...
        print(user_generated_centroids)
        print(blobs_of_id)
        ...
        for i, (blob, frame) in enumerate(zip(blobs_of_id, frames)):
            ...
            if blob is not None:
                ...
                print("blob is None")
                blob_index = np.argmin([candidate_blob.distance_from_countour_to(user_generated_centroids[i,:])
                                        for candidate_blob in self.blobs_in_video[frame]
                                        if candidate_blob.is_a_crossing])
                print(blob_index)
                nearest_blob = self.blobs_in_frame[frame][blob_index]
                print(frame, blob_index, nearest_blob)
                nearest_blob.add_centroid(video, tuple(user_generated_centroids[i,:]), id)
                print(nearest_blob.final_centroid)
