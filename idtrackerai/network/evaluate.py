        ...
        with torch.no_grad():
            if learner is not None:
                ...
                if 'weighted' in args.loss:
                    loss, output = learner.forward_with_criterion(input_, train_target, w_MCL=args.w_MCL, mask=mask)
                else:
                    loss, output = learner.forward_with_criterion(input_, train_target, mask=mask)
