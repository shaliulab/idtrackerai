...
class CEMCL_weighted(nn.Module):
    ...
    def forward(self, prob, prob1, prob2, one_hot_target, pairwise_target, w_MCL=.5):
        MCL_contrib = MCL_func(prob1, prob2, pairwise_target)
        CE_contrib = CE_func(prob, one_hot_target)
        return MCL_contrib.mul(w_MCL).add(CE_contrib.mul(1-w_MCL)), CE_contrib.mul(1-w_MCL), MCL_contrib.mul(w_MCL)
...
def CE_func(prob, target):
    ...
    assert len(prob) == len(target), 'Wrong input size:{0},{1}'.format(str(len(prob)),
                                                                       str(len(target)))
