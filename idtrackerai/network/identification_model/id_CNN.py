...
from idtrackerai.network.cnn_architectures import (
    cnn_model_0,
    cnn_model_1,
    cnn_model_2,
    cnn_model_3,
...
class ConvNetwork:
    ...
    def __init__(self, params, training_flag=True, restore_index=None):
        ...
        knowledge_transfer_info_dict = {
            "input_image_size": self.params.target_image_size
            if self.params.target_image_size is not None
            else self.params.image_size,
        ...
        np.save(
            os.path.join(self.params.save_folder, "info.npy"),
            knowledge_transfer_info_dict,
        )
        self.set_savers()
        ...
        if self.is_restoring:
            ...
            [
                self.restore_folder_conv,
                self.restore_folder_fc_softmax,
            ] = get_checkpoint_subfolders(
                self.params._restore_folder, ["conv", "softmax"]
            )
            ...
            [self.restore_folder_conv] = get_checkpoint_subfolders(
                self.params._knowledge_transfer_folder, ["conv"]
            )
    ...
    def create_summaries_writers(self):
        ...
        self.summary_writer_training = tf.summary.FileWriter(
            self.params._save_folder + "/train", self.session.graph
        )
        self.summary_writer_validation = tf.summary.FileWriter(
            self.params._save_folder + "/val", self.session.graph
        )
    ...
    def _build_graph(self):
        ...
        self.x_pl = tf.placeholder(
            tf.float32,
            [None, self.image_width, self.image_height, self.image_channels],
            name="images",
        )
        ...
        model_image_width = (
            self.image_width
            if self.params.target_image_size is None
            else self.params.target_image_size[0]
        )
        model_image_height = (
            self.image_height
            if self.params.target_image_size is None
            else self.params.target_image_size[1]
        )
        ...
        self.y_logits, self.fc_vector = CNN_MODELS_DICT[self.params.cnn_model](
            self.x_pl,
            self.params.number_of_animals,
            model_image_width,
            model_image_height,
            self.image_channels,
        )
        ...
        self.predictions = tf.cast(
            tf.add(tf.argmax(self.softmax_probs, 1), 1), tf.float32
        )

        if self.training:
            self.y_target_pl = tf.placeholder(
                tf.float32,
                [None, self.params.number_of_animals],
                name="labels",
            )
    ...
    def get_layers_to_optimize(self):
        ...
        self.layers_to_optimise = [
            layer for layers in self.layers_to_optimise for layer in layers
        ]
    ...
    def weighted_loss(self):
        ...
        cross_entropy = tf.reduce_mean(
            tf.losses.softmax_cross_entropy(
                self.y_target_pl, self.y_logits, self.loss_weights_pl
            ),
    ...
    def set_optimizer(self):
        ...
        if self.params.scopes_layers_to_optimize is not None:
            logger.debug(
                "Optimizing %s" % self.params.scopes_layers_to_optimize
            )
            self.get_layers_to_optimize()
            train_op = optimizer.minimize(
                self.loss, var_list=self.layers_to_optimise
            )
        else:
    ...
    def evaluation(self):
        ...
        """
        individual_accuracy = compute_individual_accuracy(
            self.y_target_pl, self.y_logits, self.params.number_of_animals
        )
    ...
    def restore_convolutional_layers(self):
        ...
        if self.restore_index is None:
            ckpt_path = self.check_checkpoint_path(
                ckpt.model_checkpoint_path, self.restore_folder_conv
            )
            logger.debug("Restoring convolutional part from %s" % ckpt_path)
            self.saver_conv.restore(
                self.session, ckpt_path
            )  # restore convolutional variables
        else:
            logger.debug(
                "Restoring convolutional part from %s"
                % ckpt.all_model_checkpoint_paths[self.restore_index]
            )
            self.saver_conv.restore(
                self.session,
                ckpt.all_model_checkpoint_paths[self.restore_index],
    ...
    def restore_classifier(self):
        ...
        if self.restore_index is None:
            self.saver_fc_softmax.restore(
                self.session, ckpt.model_checkpoint_path
            )  # restore fully-conected and softmax variables
            logger.debug(
                "Restoring fully-connected and softmax part from %s"
    ...
    def predict(self, batch):
        ...
        return self.session.run(
            [self.softmax_probs, self.predictions], feed_dict=feed_dict
        )
    ...
    def write_summaries(self, epoch_i, feed_dict_train, feed_dict_val):
        ...
        """
        summary_str_training = self.session.run(
            self.summary_op, feed_dict=feed_dict_train
        )
        summary_str_validation = self.session.run(
            self.summary_op, feed_dict=feed_dict_val
        )
        self.summary_writer_training.add_summary(summary_str_training, epoch_i)
        self.summary_writer_validation.add_summary(
            summary_str_validation, epoch_i
        )
...
def createSaver(name, exclude_fc_and_softmax):
    ...
    if not exclude_fc_and_softmax:
        ...
        saver = tf.train.Saver(
            [v for v in tf.global_variables() if "conv" in v.name],
            name=name,
            max_to_keep=1000000,
        )
    else:
