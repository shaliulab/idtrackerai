...
class Store_Accuracy_and_Loss(object):
    ...
    def plot(
        ...
        if canvas_from_GUI is not None:
            ...
            ax2 = axes_handles[1]
            ax1.plot(
                range(len(self.loss)), self.accuracy, color, label=self.name
            )
            width = 0.35
    ...
    def plot_global_fragments(
        self, ax_handles, video, fragments, black=False, canvas_from_GUI=None
    ):
        ...
        colors = get_spaced_colors_util(
            video.number_of_animals, norm=True, black=black
        )
        attribute_to_check = (
            "used_for_training"
            if self.scope == "training"
            else "used_for_pretraining"
        )

        for fragment in fragments:
            if (
                getattr(fragment, attribute_to_check)
                ...
                ax4.add_patch(
                    patches.Rectangle(
                        ...
                        facecolor=colors[
                            ...
                            if attribute_to_check == "used_for_training"
                            else fragment._temporary_id_for_pretraining
                        ],
                        alpha=1.0,
                    )
                )
        ...
        if (
            not hasattr(video, "maximum_number_of_blobs")
            or video.maximum_number_of_blobs == 0
        ):
        ...
        ax4.set_xlim([0.0, video.number_of_frames])
        ax4.set_ylim([-0.5, max_y + 0.5 - 1])
