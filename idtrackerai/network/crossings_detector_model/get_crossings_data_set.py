...
class CrossingDataset(object):
    def __init__(
        ...
        if scope == "training" or scope == "validation":
            ...
            if self.there_are_crossings:
                self.get_list_of_individual_blobs_for_training(
                    individual_blobs
                )
                logger.info(
                    "number of sure crossing images used for training: %i"
                    % len(self.crossing_blobs)
                )
                logger.info(
                    "number of individual images used for training: %i"
                    % len(self.individual_blobs)
                )
        if scope == "test":
    ...
    def get_list_of_crossing_blobs_for_training(self, crossings, image_size):
        if len(crossings) == 0 or image_size is None:
            ...
            logger.debug(
                "number of crossing blobs (in get list): %i"
                % len(self.crossing_blobs)
            )
            if (
                len(self.crossing_blobs)
                ...
                self.image_size = (
                    np.max(
                        [
                            np.max(crossing.bounding_box_image.shape)
                            for crossing in self.crossing_blobs
                        ]
                    )
                    + 5
                )
    ...
    def get_data(self, sampling_ratio_start=0, sampling_ratio_end=1.0):
        logger.info("Generating crossing %s set." % self.scope)
        self.crossings_sliced = self.slice(
            self.crossing_blobs, sampling_ratio_start, sampling_ratio_end
        )
        ...
        if self.scope == "training":
            self.crossings_images, self.crossing_labels = duplicate_PCA_images(
                self.crossings_images, self.crossing_labels
            )
        assert len(self.crossing_labels) == len(self.crossings_images)
        logger.info("Generating single individual %s set" % self.scope)
        self.individual_blobs_sliced = self.slice(
            self.individual_blobs, sampling_ratio_start, sampling_ratio_end
        )
        self.individual_blobs_images = self.generate_individual_blobs_images()
        self.individual_blobs_labels = np.zeros(
            len(self.individual_blobs_images)
        )
        assert len(self.individual_blobs_labels) == len(
            self.individual_blobs_images
        )
        logger.info("Preparing images and labels")
        self.images = np.asarray(
            list(self.crossings_images) + list(self.individual_blobs_images)
        )
        self.images = np.expand_dims(self.images, axis=3)
        self.labels = np.concatenate(
            [self.crossing_labels, self.individual_blobs_labels], axis=0
        )
        self.labels = self.dense_to_one_hot(
            np.expand_dims(self.labels, axis=1)
        )
        assert len(self.images) == len(self.labels)
        if self.scope == "training":
            self.weight_positive = 1 - len(self.crossing_labels) / len(
                self.labels
            )
        else:
    ...
    def generate_crossing_images(self):
        ...
        logger.debug(
            "resampling factor crossings: %s" % str(self.resampling_factor)
        )

        for crossing in self.crossings_sliced:
            _, _, _, crossing_image = crossing.get_image_for_identification(
                self.video, image_size=self.image_size
            )
            crossing_image = cv2.resize(
                crossing_image,
                None,
            ...
            crossing_image = (
                (crossing_image - np.mean(crossing_image))
                / np.std(crossing_image)
            ).astype("float32")
    ...
    def generate_individual_blobs_images(self):
        ...
        self.compute_resampling_factor()
        logger.debug(
            "resampling factor individual: %s" % str(self.resampling_factor)
        )
        ...
        for individual_blobs in self.individual_blobs_sliced:
            (
                ...
                _,
                _,
                individual_blobs_image,
            ) = individual_blobs.get_image_for_identification(
                self.video, image_size=self.image_size
            )
            individual_blobs_image = cv2.resize(
                individual_blobs_image,
                None,
                fx=self.resampling_factor,
                fy=self.resampling_factor,
            ...
            individual_blobs_image = (
                (individual_blobs_image - np.mean(individual_blobs_image))
                / np.std(individual_blobs_image)
            ).astype("float32")
    ...
    def generate_test_images(self, interval=None):
        ...
        self.compute_resampling_factor()
        logger.debug(
            "resampling factor test: %s" % str(self.resampling_factor)
        )
        if interval is None:
            blobs = self.test
        else:
        ...
        for blob in blobs:
            _, _, _, test_image = blob.get_image_for_identification(
                self.video, image_size=self.image_size
            )
            ...
            test_image = (
                (test_image - np.mean(test_image)) / np.std(test_image)
            ).astype("float32")
