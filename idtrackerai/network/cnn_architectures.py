...
def cnn_model_0(images, classes, width, height, channels):
    ...
    pool2 = 2
    pad2 = "SAME"
    max_pool2, w2, h2 = maxpool2d(
        "maxpool1", w1, h1, relu1, pool2, stride2, pad2
    )
    d2 = n_filter1
    # conv2
    filter_size3 = 5
    ...
    stride3 = [1, 1, 1, 1]
    pad3 = "SAME"
    conv3, w3, h3, W3 = buildConv2D(
        "conv2", w2, h2, d2, max_pool2, filter_size3, n_filter3, stride3, pad3
    )
    # relu
    ...
    pool4 = 2
    pad4 = "SAME"
    max_pool4, w4, h4 = maxpool2d(
        "maxpool2", w3, h3, relu2, pool4, stride4, pad4
    )
    d4 = n_filter3
    # conv4
    filter_size5 = 5
    ...
    stride5 = [1, 1, 1, 1]
    pad5 = "SAME"
    conv5, w5, h5, W5 = buildConv2D(
        "conv3", w4, h4, d4, max_pool4, filter_size5, n_filter5, stride5, pad5
    )
    d5 = n_filter5
    # relu
    ...
    conv5_flat = tf.reshape(
        relu3, [-1, resolutionS * d5], name="conv5_reshape"
    )
    # fully-connected 1
    n_fc = 100
    fc_drop, WFC = buildFc(
        "fully-connected1", conv5_flat, w5, h5, d5, n_fc, conf.KEEP_PROB
    )
    relu = reLU("relu4", fc_drop)
    y_logits, WSoft = buildSoftMax(
        "fully_connected_pre_softmax", relu, n_fc, classes
    )
...
def cnn_model_1(images, classes, width, height, channels):
    ...
    conv1_flat = tf.reshape(
        relu1, [-1, resolutionS * n_filter1], name="conv5_reshape"
    )
    # fully-connected 1
    n_fc = 100
    fc_drop, WFC = buildFc(
        "fully-connected1", conv1_flat, w1, h1, n_filter1, n_fc, conf.KEEP_PROB
    )
    relu = reLU("relu4", fc_drop)
    y_logits, WSoft = buildSoftMax(
        "fully_connected_pre_softmax", relu, n_fc, classes
...
def cnn_model_2(images, classes, width, height, channels):
    ...
    conv3_flat = tf.reshape(
        relu2, [-1, resolutionS * n_filter3], name="conv5_reshape"
    )
    # fully-connected 1
    n_fc = 100
    fc_drop, WFC = buildFc(
        "fully-connected1", conv3_flat, w3, h3, n_filter3, n_fc, conf.KEEP_PROB
    )
    relu = reLU("relu4", fc_drop)
    y_logits, WSoft = buildSoftMax(
        "fully_connected_pre_softmax", relu, n_fc, classes
...
def cnn_model_3(images, classes, width, height, channels):
    ...
    pool6 = 2
    pad6 = "SAME"
    max_pool6, w6, h6 = maxpool2d(
        "maxpool3", w5, h5, relu3, pool6, stride6, pad6
    )
    d6 = n_filter5
    # conv4
    filter_size7 = 5
    ...
    stride7 = [1, 1, 1, 1]
    pad7 = "SAME"
    conv7, w7, h7, W7 = buildConv2D(
        "conv4", w6, h6, d6, max_pool6, filter_size7, n_filter7, stride7, pad7
    )
    d7 = n_filter7
    # relu
    ...
    # linearize weights for fully-connected layer
    resolutionS = w7 * h7
    conv7_flat = tf.reshape(
        relu8, [-1, resolutionS * d7], name="conv7_reshape"
    )
    # fully-connected 1
    n_fc = 100
    fc_drop, WFC = buildFc(
        "fully-connected1", conv7_flat, w7, h7, d7, n_fc, conf.KEEP_PROB
    )
    relu = reLU("relu4", fc_drop)
    y_logits, WSoft = buildSoftMax(
        "fully_connected_pre_softmax", relu, n_fc, classes
...
def cnn_model_5(images, classes, width, height, channels):
    ...
    pad2 = "SAME"
    max_pool2, w2, h2 = maxpool2d(
        "maxpool1", w1, h1, conv1, pool2, stride2, pad2
    )
    d2 = n_filter1
    # conv2
    filter_size3 = 5
    ...
    pad4 = "SAME"
    max_pool4, w4, h4 = maxpool2d(
        "maxpool2", w3, h3, conv3, pool4, stride4, pad4
    )
    d4 = n_filter3
    # conv4
    filter_size5 = 5
    ...
    resolutionS = w5 * h5
    conv5_flat = tf.reshape(
        conv5, [-1, resolutionS * d5], name="conv5_reshape"
    )
    # fully-connected 1
    n_fc = 100
    fc_drop, WFC = buildFc(
    ...
    y_logits, WSoft = buildSoftMax(
        "fully_connected_pre_softmax", fc_drop, n_fc, classes
    )
...
def cnn_model_8(images, classes, width, height, channels):
    ...
    # fully-connected 1
    n_fc_1 = 100
    fc_drop_1, WFC = buildFc(
        "fully-connected1", conv5_flat, w5, h5, d5, n_fc_1, conf.KEEP_PROB
    )
    ...
    y_logits, WSoft = buildSoftMax(
        "fully_connected_pre_softmax", relu, n_fc_2, classes
    )
...
def cnn_model_crossing_detector(images, classes, width, height, channels):
    ...
    n_fc = 100
    fc_drop, WFC = buildFc(
        "fully-connected1", conv5_flat, w3, h3, n_filter3, n_fc, conf.KEEP_PROB
    )
