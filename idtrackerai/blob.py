...
class Blob(object):
    ...
    bounding_box_in_frame_coordinates: list
        List of tuples [(x, y), (x + width, y + height)] of the bounding box
        rectangle enclosing the blob's pixels
    ...
    _is_a_misclassified_individual : bool
        This property can be modified only by the user during validation. It
        identifies a blob that was by mistaken associated to a crossing by the
        DeepCrossingDetector
    next : list
        List of blob objects segmented in self.frame_number + 1 whose list of
        pixels intersect self.pixels
    previous : list
        List of blob objects segmented in self.frame_number - 1 whose list of
        pixels intersect self.pixels
    _fragment_identifier : int
        Unique integer identifying the fragment (built by blob overlapping) to
        which self belongs to
    _blob_index : int
        Hierarchy of the blob at the beggining of the core of the global fragment.
        Only used to plot accumulation steps
    _used_for_training : bool
        If True the image obtained from the blob has been used to train the
        idCNN
    _accumulation_step : int
        Accumulation step in which the image associated to the blob has been
        accumulated
    ...
    assigned_identities : list
        Identity assigned to self by the algorithm (ignoring eventual
        correction made by the user during validation)
    ...
    def __init__(
        ...
        # path to where the bounding_box_image is saved
    ...
    def distance_from_countour_to(self, point):
        r"""Returns the distance between the point passed as input and the closes
    ...
    def add_centroid(
        ...
        ----------
        centroid : tuple
            centroid to be added in full resolution coordinates.
            len(centroid) must be 2
        identity : int
    ...
    def propagate_identity(self, old_identity, new_blob_identity, centroid):
        ...
        while (
            ...
            if len(current.next[0].final_centroids) > 1:
                ...
                else:  # there are several centroids in the blob with the same
                    # identity
    ...
    def draw(
        ...
        for i, (identity, centroid) in enumerate(
            ...
            if identity is not None:
                ...
                # cv2.putText(image, idstr, str_pos, cv2.FONT_HERSHEY_SIMPLEX,
...
def full2miniframe(point, boundingBox):
    r"""Maps a point in the fullframe to the coordinate system defined by the image
