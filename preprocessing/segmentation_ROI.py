...
def segmentAndSave(path, height, width, mask, selectROI, bkg, bkgSubstraction, EQ, minThreshold, maxThreshold, minArea, maxArea):
...
def segment(paths, name, numAnimals,
    ...
    ret, screenshotROI = cap.read()
    screenshotROI = cv2.cvtColor(screenshotROI, cv2.COLOR_BGR2GRAY)
    cap.release()

    mask, centers = checkROI(selectROI, screenshotROI)
    ...
    OupPutParallel = Parallel(n_jobs=num_cores)(delayed(segmentAndSave)(path, height, width, mask, selectROI, bkg, bkgSubstraction, EQ, minThreshold,maxThreshold,minArea,maxArea) for path in paths)
