...
def idFragment(IdProbs):
    ...
    for IdProb in IdProbs:
        # probFragmentsId = np.subtract(1, np.prod(np.subtract(1.0001, IdProb), axis = 0))
...
if __name__ == '__main__':
    ...
    filename = filename.split('_')[0] + '_videoInfo.pkl'
    videoInfo = pd.read_pickle(filename)
    ...
    if train == 0:
        ...
        AllFragmentsIds = []
        AllProbFragmentsIds = []
        # allIdentities = pd.DataFrame(index=portraits.index,columns=range(maxNumBlobs))
        # allProbabilities = pd.DataFrame(index=portraits.index,columns=range(maxNumBlobs))
        ...
        for i in range(maxNumBlobs):
            ...
            # AllProbFragmentsIds.append(probFragmentsIds)
            # AllFragmentsIds.append(fragmentsIds)
            #
            #
            # AllIds.append(Ids)
            # AllIdProbs.append(idProbs)
