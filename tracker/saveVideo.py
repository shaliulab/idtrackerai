...
def IdSaver(paths,allIdentities,frameIndices, show=True):
    path = paths[0]
    video = os.path.basename(path)
    filename, extension = os.path.splitext(video)
    folder = os.path.dirname(path)
    # videoInfo = pd.read_pickle('../Conflict8/conflict3and4_videoInfo.pkl')
    videoInfo = pd.read_pickle('../Cafeina5peces/Caffeine5fish_videoInfo.pkl')
    ...
    for path in paths:
        ...
        while currentFrame < numFrame:
            ...
            currentFrame += 1
IdSaver(paths,allIdentities,frameIndices, show=False)
