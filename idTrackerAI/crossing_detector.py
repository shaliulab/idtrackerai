...
def check_if_there_is_a_crossing(list_of_blobs):
    there_is_a_crossing = False
    frame_number = 0

    while not there_is_a_crossing and frame_number < len(list_of_blobs.blobs_in_video):
        there_is_a_crossing = np.any([blob.is_a_sure_crossing() for blob in list_of_blobs.blobs_in_video[frame_number]])
        frame_number += 1
    return there_is_a_crossing
...
def detect_crossings(list_of_blobs, video, model_area):
    ...
    if check_if_there_is_a_crossing(list_of_blobs):
