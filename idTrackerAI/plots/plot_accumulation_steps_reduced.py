...
def set_properties_final_accuracy(video, fig, ax_P2, ax_gt_accuracy, number_of_accumulation_steps):
    for ax in [ax_P2, ax_gt_accuracy]:
        pos = ax.get_position()
        ax.set_position([pos.x0 + .05, pos.y0 + 0.015, pos.width - 0.05, pos.height - 0.025])

    fig.text(pos.x0, pos.y0 + 0.025, 'Number of individuals', ha='left', va='bottom', rotation='vertical', fontsize = 12)
...
def plot_accumulation_steps(video, list_of_fragments, list_of_blobs, list_of_blobs_no_gaps, training_dict, validation_dict):
    ...
    ### Final certainty
    ax_P2 = plt.subplot2grid((number_of_accumulation_steps_to_plot + 2, 5), (2, 3), colspan=2)
    ax_gt_accuracy = plt.subplot2grid((number_of_accumulation_steps_to_plot + 2, 5), (3, 3), colspan=2)
