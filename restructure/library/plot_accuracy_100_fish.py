...
if __name__ == '__main__':
    ...
    results_Pretrain_Accum05_Gaussian = pd.read_pickle('/home/rhea/Desktop/IdTrackerDeep/restructure/library/results_data_frame_pretrain_accum_100fish_3000frames_gaussian_certainty05.pkl' )
    ...
    ac5 = np.mean(np.reshape(np.asarray(results_Pretrain_Accum05_Gaussian.accuracy),(5,3)),axis=1)

    accs = zip(ac0, ac1, ac2, ac3, ac4, ac5)
    ...
    for i, mean_frames in enumerate(mean_frames_per_fragment):
        ax.plot([0,1,2,3,4,5],accs[i],label = str(mean_frames_per_fragment[i]))

    ax.legend()
    ax.set_xlabel('condition')
    ax.set_ylabel('accuracy')
    plt.xticks([0,1,2,3,4,5], ['noPretain-noAccum-noGaussian',
                                'noPretain-noAccum-Gaussian',
                                'noPretrain-Accum-certainty0.1-Gaussian',
                                'noPretrain-Accum-certainty0.5-Gaussian',
                                'pretrain-noAccum-Gaussian',
                                'pretrain_Accum_certainty0.5-Gaussian'])
